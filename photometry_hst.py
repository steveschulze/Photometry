...
try:
    ...
    science.add_row(['MJD', np.nan, np.nan, np.nan, np.round(time.Time(hdu_header['DATE-OBS'], format='isot', scale='utc').mjd, 7)])
except:
    science.add_row([key,   np.nan, np.nan, np.nan, '...'])
    science.add_row(['MJD', np.nan, np.nan, np.nan, '...'])
...
science.add_row(['NCOMBINE', np.nan, np.nan, np.nan, '1'])
