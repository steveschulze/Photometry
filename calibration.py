...
def make_poststamp(
    ...
    ellipses: 'list[tuple[float, float, float, str]] | None' = None,
) -> None:
    ...
    ``aperture_radii`` / ``ellipses`` are given, the measurement apertures are
    overlaid on the left panel so the user can judge visually which aperture
    best captures the target (SExtractor-style check image; ``sep`` produces
    none).  Fixed circular apertures are drawn solid; the elliptical Kron
    (AUTO) and Petrosian apertures are drawn dashed.
    ...
    aperture_radii : np.ndarray | None
        Fixed-aperture **radii** in pixels.  Drawn as concentric solid circles
    ...
    aperture_labels : list[str] | None
        Legend labels for each circular aperture (same length as
        *aperture_radii*).  Defaults to the diameter in pixels if omitted.
    ellipses : list[tuple[float, float, float, str]] | None
        Elliptical apertures (Kron/AUTO, Petrosian) as
        ``(width, height, angle_deg, label)`` tuples — full axis lengths in
        pixels and position angle in degrees.  Drawn dashed, centred on the
        same measured position.  Typically only for a detected source.
    """
    fits_path  = Path(fits_path)
    ...
    has_circles  = aperture_radii is not None and len(aperture_radii)
    has_ellipses = ellipses is not None and len(ellipses)
    if has_circles or has_ellipses:
        # Centre on the position actually measured (observed centroid if
        # detected, else the expected position).
        if coord_obs[0] is not None:
        ...
        # Fixed circular apertures — solid.  High-contrast hues that avoid the
        # reserved green (observed) and blue (expected) markers, cycled if many.
        if has_circles:
            ...
            for j, r in enumerate(radii):
                ...
                c = Circle((tx, ty), radius=float(r), fill=False, lw=3,
                           edgecolor=palette[j % len(palette)], label=lbl)
                ax_chk.add_patch(c)
                handles.append(c)

        # Elliptical Kron (AUTO) / Petrosian apertures — dashed.
        if has_ellipses:
            ell_colors = ['#17becf', '#0b7285', '#000000']
            for k, (w, h, ang, lbl) in enumerate(ellipses):
                e = Ellipse((tx, ty), width=float(w), height=float(h),
                            angle=float(ang), fill=False, lw=3, ls='--',
                            edgecolor=ell_colors[k % len(ell_colors)], label=lbl)
                ax_chk.add_patch(e)
                handles.append(e)
